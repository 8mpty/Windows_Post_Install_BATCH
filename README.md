...
```
irm 8mpty.com/win/post | iex
```
or (A bit more Secure)
```
irm https://8mpty.com/win/post | iex
```
...
Default is ```irm 8mpty.com/win/post | iex```,

But if you want to pass in arguments:
```
iex "&{$(irm 8mpty.com/win/post)} 5"
```
Which will go to the [Extras] page.

If running:
```
iex "&{$(irm 8mpty.com/win/post)} 5 6"
```
Will go the [Extras] page and execute option [6] which is creating the script shortcut.
</p>
</details>
...
EX. ```iex "&{$(irm 8mpty.com/win/post)} 5 4 2 5 3 1 1 7 0"```
